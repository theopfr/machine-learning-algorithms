...
class KNN:
    ...
    def test(self):
        ...
        example_idx = np.random.randint(0, len(self.test_x - 1))
        example_sample, example_target = self.test_x[example_idx], self.test_y[example_idx]
        prediction = self._classify_datapoint(self.train_x, self.train_y, example_sample)
        
        if list(prediction) == [1, 0, 0]: 
            self.plot_dataset(point=((example_sample[0], example_sample[1], example_sample[2], "green")), title="is: " + str(example_target) + " got: " + str(prediction))
        elif list(prediction) == [0, 1, 0]: 
            self.plot_dataset(point=((example_sample[0], example_sample[1], example_sample[2], "blue")), title="is: " + str(example_target) + " got: " + str(prediction))
        elif list(prediction) == [0, 0, 1]: 
            self.plot_dataset(point=((example_sample[0], example_sample[1], example_sample[2], "red")), title="is: " + str(example_target) + " got: " + str(prediction))

    
    """ plot dataset """
    def plot_dataset(self, point: tuple=(), title: str=""):
        ...
        ax.scatter(setosa_x, setosa_y, setosa_z, color="green", alpha=0.35)
        ax.scatter(versicolor_x, versicolor_y, versicolor_z, color="blue", alpha=0.35)
        ax.scatter(virignica_x, virignica_y, virignica_z, color="red", alpha=0.35)

        if point[0] is not None:
            ax.scatter(point[0], point[1], point[2], color=point[3], s=100)
...
knn = KNN(K=5)
# knn.plot_dataset(title="dataset")
knn.test()
